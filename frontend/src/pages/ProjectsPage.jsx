...
const ProjectsPage = () => {
  ...
  const loadProjects = async (currentCategory, currentSearch) => {
    ...
    try {
      ...
      setError(getErrorMessage(requestError, 'Unable to load projects right now.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const fallbackProjects = SIGNATURE_PROJECTS.filter((project) => {
    ...
    const searchable = `${project.title} ${project.tagline} ${project.description} ${(project.techStack || []).join(' ')}`.toLowerCase()
    ...
    <>
      ...
      <section className="section-wrap pt-12 sm:pt-20">
        ...
        <p className="mt-4 max-w-3xl text-sm text-slate-400">
          This section combines live backend data with curated signature project narratives from my verified portfolio profile.
        </p>
        ...
        <div className="mt-8">
          {loading ? <LoadingState message="Loading projects..." cards={6} /> : null}
          {!loading && error ? <ErrorState message={error} onRetry={() => loadProjects(category, search)} /> : null}
